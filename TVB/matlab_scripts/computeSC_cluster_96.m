function computeSC_cluster_96(filesDIR,roi,outfile)
...
%load('/scratch/mcintosh_lab/jwang/local/subjects/021_S_4718_96/mrtrix_96/masks_96/affine_matrix.mat','-ascii');
load([filesDIR '/affine_matrix.mat']);
%wmborder.img = load('/scratch/mcintosh_lab/jwang/local/subjects/021_S_4718_96/mrtrix_96/masks_96/wmborder.mat');
wmborder.img=load([filesDIR '/wmborder.mat']);
%matlabpool open 8
    ...
    for tile = 1:length(tilefiles),
        if tilefiles(tile).bytes > 1000,
            ...
            for trackind = 1:length(tracks.data),
                ...
                regionIndVec=find(pathids==region_table(region));

                %if ~isempty(inregids), %Check if the Path has Points on the Border
                if ~isempty(regionIndVec),
                    %tracklen=size(tracks.data{1,trackind},1)-inregids(end); %Measure the length from the Endpoint to the last Point that exits the starting Region
                     tracklen_1=size(tracks.data{1,trackind},1)-regionIndVec(end);
                     tracklen_2=regionIndVec(1);
                    if tracklen_1 > 40 & any(abs(pathinds(end)-region_table)< 0.2), %Check if the track has a minimum length (step-size is 0.2mm)
                        %if pathids(end) ~= 0, %Check if the Path has a valid endpoint
                           % if region_table(region) == pathids(inregids(end)) | region_table(region) == pathids(end), %Check if the Region-ID requested matches the Seedpoint-Region
                                good_tracks=good_tracks+1; %"[...] when you have eliminated the impossible, whatever remains, however improbable, must be the truth"

                                
                                seed_id=find(region_id_table(:,2) == pathinds(regionIndVec(end)));
                                ...
                                r2=find(region_table==pathids(regionIndVec(end)));

                                SC_dist(r1,r2).dist=[SC_dist(r1,r2).dist;tracklen_1]; %Add the distance of the current track to a pool of distances between the two ROIS
                                SC_dist(r2,r1).dist=[SC_dist(r2,r1).dist;tracklen_1];

                                % New distances computation
                                SC_dist_new(seed_id).e=[SC_dist_new(seed_id).e;tracklen_1]; %Add a Connection from Seedvoxel to Targetvoxel
                                SC_dist_new(target_id).e=[SC_dist_new(target_id).e;tracklen_1]; %Add a Connection from Targetvoxel to Seedvoxel
                    ...
                    if tracklen_2 > 40 & any(abs(pathids(1)-region_table)< 0.2), %Check if the track has a minimum length (step-size is 0.2mm)
                        %if pathids(end) ~= 0, %Check if the Path has a valid endpoint
                           % if region_table(region) == pathids(inregids(end)) | region_table(region) == pathids(end), %Check if the Region-ID requested matches the Seedpoint-Region
                                good_tracks=good_tracks+1; %"[...] when you have eliminated the impossible, whatever remains, however improbable, must be the truth"

                                
                                seed_id=find(region_id_table(:,2) == pathinds(regionIndVec(1)));
                                target_id = find(region_id_table(:,2)==pathinds(1));
                                ...
                                % Old distances computation
                                r1=find(region_table==pathids(1)); %Transfer the Indexnr. from Desikan-Numbering (i.e. 1001-2035) to a Matrix-Numbering (i.e. 1-68)
                                r2=find(region_table==pathids(regionIndVec(1)));

                                SC_dist(r1,r2).dist=[SC_dist(r1,r2).dist;tracklen_2]; %Add the distance of the current track to a pool of distances between the two ROIS
                                SC_dist(r2,r1).dist=[SC_dist(r2,r1).dist;tracklen_2];

                                % New distances computation
                                SC_dist_new(seed_id).e=[SC_dist_new(seed_id).e;tracklen_2]; %Add a Connection from Seedvoxel to Targetvoxel
                                SC_dist_new(target_id).e=[SC_dist_new(target_id).e;tracklen_2]; %Add a Connection from Targetvoxel to Seedvoxel
