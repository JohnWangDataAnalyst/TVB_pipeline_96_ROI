...
#(commented out by authors)mri_convert --in_orientation LIA --out_orientation RAS wm_outline.nii wm_outline_${numROI}.nii
gzip wm_outline_${numROI}.nii 
...
if [ "$numROI" == "96" ]
then
roimask=/home/jwang/data/glebs/rmask_gleb_1mm.nii.gz
...
mri_convert --in_type mgz --out_type nii --out_orientation RAS ${path}/${pfx}/recon_all/mri/brainmask.mgz ${path}/${pfx}/recon_all/mri/brainmask.nii.gz
T1_brain=${path}/${pfx}/recon_all/mri/brainmask.nii.gz
mri_convert --in_type mgz --out_type nii --out_orientation RAS ${path}/${pfx}/recon_all/mri/T1.mgz ${path}/${pfx}/recon_all/mri/T1.nii.gz
T1=${path}/${pfx}/recon_all/mri/T1.nii.gz
...
#{path}/${pfx}/calc_images/
cp $(rootPath)/matlab_scripts/*.m ${subfolder}/${subID}/calc_images
cp ${subfolder}/${subID}/recon_all/mri/aparc+aseg.nii ${subfolder}/${subID}/calc_images
gzip aparc+aseg.nii
octave --eval "refineGlebs('./')"
...
#Combine & Binarize
fslmaths wmmask_${numROI}.nii.gz -add wmoutline2diff.nii -bin wmmask_{numROI}.nii.gz
